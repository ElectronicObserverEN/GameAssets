/*
	The script is intentionally sync to avoid problems with making too many requests at once.
 */

const string fileSavePath = @"E:\Testing";
const string imageSourceUrl = "https://raw.githubusercontent.com/Nishisonic/gkcoi/master/static/ship";
...
foreach (int shipId in shipIds)
{
	foreach (string resourceType in resourceTypes)
	{
		HttpResponseMessage response = await client
			.GetAsync(Path.Combine(imageSourceUrl, resourceType, $"{shipId}.png"));
...
int CreateKey(string? t) => t?.Sum(c => c) ?? 0;
...
*/
string GetShipImagePath(int shipId, bool damaged, string imageType)
{
	string folderName = imageType + (damaged ? "_dmg" : "");
	string fullImageType = "ship_" + folderName;
	string imageHash = CreateHash(shipId, fullImageType);
